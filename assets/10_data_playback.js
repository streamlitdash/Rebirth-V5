...
(() => {
  ...
  const dataPlayback = (
    ...
    sliderValue, resetGeneration, cacheState, rawRows, rawColumns,
    visibilityState, playerState,
  ) => {
    ...
    const selectedRows = (Array.isArray(rawRows) ? rawRows : []).filter(
      (record) => record && String(record[dateColumn] ?? "") === selectedDate,
    );
    const selectedColumns = (Array.isArray(rawColumns) ? rawColumns : []).filter(
