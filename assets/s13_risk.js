...
(() => {
  ...
  document.addEventListener("click", (event) => {
    ...
    const promotionTrigger = event.target.closest(
      "#promotion-recalculate-current-view"
    );
    if (promotionTrigger && !promotionTrigger.disabled) {
      // Let the native click publish n_clicks before reflecting its busy state.
      window.setTimeout(() => {
        const setProps = window.dash_clientside?.set_props;
        if (typeof setProps === "function") {
          setProps("promotion-recalculate-current-view", {
            children: "Recalculating…",
            disabled: true,
            "aria-busy": "true",
          });
          setProps("promotion-generation-status", {
            children: "Recalculating promotion for the visible scope…",
            className: "filter-note is-running",
          });
        }
      }, 0);
    }
      ...
      if (mode === "commo" || mode === "checker") {
        ...
        if (typeof setProps === "function") {
          setProps("data-settings-status", {
            children: mode === "commo"
              ? "Applying commodity quote setting…"
              : "Applying risk-date setting…",
          });
        }
      }
