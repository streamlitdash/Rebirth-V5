"""Lazy site-owned connector boundary over explicit fake CSV fixtures.
...
from rebirth.adapters.s06_crossgamma import get_cross_gamma as get_cross_gamma_matrix
...
def _fake_csv_revision(dataset: str) -> tuple[Path, str, int, int]:
    ...
    try:
        ...
        raise FakeCsvConnectorError(
            ...
            f"{dataset!r} loader in rebirth.services.s05_sources with a real function."
        ) from exc
...
def get_portfolio_config(portfolio_date: pd.Timestamp) -> pd.DataFrame:
    ...
    ``rebirth.domain.s01_schema.PORTFOLIO_CONFIG_REQUIRED_COLUMNS``. Optional registered
...
def send_sog_pl(frame: pd.DataFrame) -> None:
    ...
    raise RuntimeError(
        ...
        "replace rebirth.services.s05_sources.send_sog_pl for an authorized deployment"
    )
...
def send_portfolio_pl(frame: pd.DataFrame) -> None:
    ...
    raise RuntimeError(
        ...
        "replace rebirth.services.s05_sources.send_portfolio_pl for an authorized deployment"
    )
