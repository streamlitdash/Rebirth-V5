...
from rebirth.ui.aggregation import apply_credit_measure, ordered_unique
...
from .promotion import PROMOTION_GENERATION_STORE_ID
from .state import risk_exclude_selected
...
def register_pivot_callbacks(
    ...
    def manage_pivot_editor(
        ...
        return (
            ...
            *pivot_control_values(spec),
            "Loaded. Edit fields and select Apply when ready.",
        )
    ...
    def load_pivot_filter_values(
        ...
        return options, [value for value in requested if value in available]
    ...
    def render_custom_pivot(
        ...
        try:
            ...
            def build_render():
                ...
                status = (
                    f"Rows {row_start:,}–{row_end:,} of {result.row_count:,} · "
                    f"Columns {column_start:,}–{column_end:,} of "
                    f"{result.column_count:,}"
                )
                return (
                    build_native_pivot_table(result),
                    status,
