...
from rebirth.domain.products import AxisSpec, PRODUCT_SPECS_BY_SOURCE_TYPE, ProductSpec
...
class HistoryCatalogEntry:
    """One archive-backed identity available to direct Data selection."""
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.kind, str) or self.kind.strip().casefold() not in {
            "risk",
            "market",
        }:
            raise HistoryValidationError("catalog kind must be 'risk' or 'market'")
        kind = self.kind.strip().casefold()
        if not isinstance(self.identity, HistoryIdentity):
            raise HistoryValidationError("catalog identity must be a HistoryIdentity")
        if kind == "market" and (
            self.identity.identity_mode != "underlying"
            or len(self.identity.source_types) != 1
        ):
            raise HistoryValidationError(
                "Market catalog entries require one raw Underlying Source Type"
            )
        object.__setattr__(self, "kind", kind)
        object.__setattr__(
            self,
            "source_revision",
            _nonnegative_int(self.source_revision, label="catalog source_revision"),
        )
        object.__setattr__(
            self,
            "snapshot_date",
            _date(self.snapshot_date, label="catalog snapshot_date"),
        )
    ...
    def to_handoff(self, *, reset_generation: int = 0) -> HistoryHandoff:
        return HistoryHandoff(
            schema_version=HISTORY_HANDOFF_SCHEMA_VERSION,
            kind=self.kind,
            identity=self.identity,
            metric="risk" if self.kind == "risk" else "current",
            source_revision=self.source_revision,
    ...
    @classmethod
    def from_mapping(cls, value: object) -> HistoryCatalogEntry:
        if not isinstance(value, Mapping):
            raise HistoryValidationError("history catalog entry must be a mapping")
        _exact_keys(
            value,
            {"key", "kind", "identity", "source_revision", "snapshot_date"},
            label="history catalog entry",
        )
        entry = cls(
            kind=value["kind"],
            identity=HistoryIdentity.from_mapping(value["identity"]),
            ...
            snapshot_date=_date(
                value["snapshot_date"],
                label="catalog snapshot_date",
                strict_text=True,
            ),
        )
        if value["key"] != entry.key:
            raise HistoryValidationError("history catalog entry key is invalid")
        return entry


@dataclass(frozen=True)
class HistoryIdentityCatalog:
    """Small immutable identity index for one validated archive generation."""

    generation: str
    entries: tuple[HistoryCatalogEntry, ...]

    def __post_init__(self) -> None:
        generation = _text(self.generation, label="catalog generation")
        if not isinstance(self.entries, tuple) or any(
            not isinstance(entry, HistoryCatalogEntry) for entry in self.entries
        ):
            raise HistoryValidationError(
                "catalog entries must be an immutable HistoryCatalogEntry tuple"
            )
        keys = [entry.key for entry in self.entries]
        if len(keys) != len(set(keys)):
            raise HistoryValidationError("history catalog contains duplicate entries")
        object.__setattr__(self, "generation", generation)

    def resolve(self, key: object) -> HistoryCatalogEntry:
        selected = _text(key, label="history catalog key")
        for entry in self.entries:
            if entry.key == selected:
                return entry
        raise HistoryValidationError("selected history identity is not in the archive")
    ...
    @classmethod
    def from_mapping(cls, value: object) -> HistoryIdentityCatalog:
        if not isinstance(value, Mapping):
            raise HistoryValidationError("history catalog must be a mapping")
        _exact_keys(value, {"generation", "entries"}, label="history catalog")
        raw_entries = value["entries"]
        if isinstance(raw_entries, (str, bytes)) or not isinstance(
            raw_entries, Sequence
        ):
            raise HistoryValidationError("history catalog entries must be a sequence")
        return cls(
            generation=value["generation"],
            entries=tuple(
                HistoryCatalogEntry.from_mapping(entry) for entry in raw_entries
            ),
        )
