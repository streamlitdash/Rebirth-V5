"""Focused Rebirth V4.1 native-pivot and Custom Risk View tests."""
...
from rebirth.domain import s11_riskviews as risk_views_module
from rebirth.domain.s11_riskviews import CROSS_PIVOT_SPEC, PivotSpec, RiskViewRepository
from rebirth.pages.risk.s13_pivot import (
    build_hierarchical_pivot_table,
    build_native_pivot_table,
    compute_native_pivot,
    pivot_spec_from_controls,
)
from rebirth.pages.risk.s14_pivotcallbacks import (
    mutate_risk_view,
    pivot_filter_status,
...
def test_custom_pivot_reported_underlying_filter_and_chevrons_share_tree_logic() -> (
    None
):
    raw = _raw_frame()
    raw.loc[raw.index % 2 == 1, "Reported Underlying"] = "EUR-ESTR"
    raw.loc[raw.index % 2 == 1, "Underlying"] = "EUR-ESTR"
    frame = prepare_risk_data(raw)
    spec = pivot_spec_from_controls(
        rows=["activity", "portfolio"],
        columns=[],
        measures=["risk"],
        filter_field="reported underlying",
        filter_values=["USD-SOFR"],
        sort_field=None,
        sort_direction="asc",
        totals=["grand"],
        row_limit=20,
    ...
    result = compute_native_pivot(frame, spec)
    collapsed = build_hierarchical_pivot_table(
        frame,
        result,
        open_rows=[],
        view_token="custom-test",
    )
    collapsed_rows = [
        item
        for item in _walk(collapsed)
        if "group-row" in str(getattr(item, "className", "")).split()
    ]
    root = collapsed_rows[0]
    toggle = next(
        item for item in _walk(root) if getattr(item, "className", None) == "row-toggle"
    )
    root_key = getattr(root, "data-risk-key")
    expanded = build_hierarchical_pivot_table(
        frame,
        result,
        open_rows=[root_key],
        view_token="custom-test",
    ...
    labels = [
        ...
        if getattr(item, "className", None) == "row-label-text"
    ]

    assert result.row_count == 8
    assert getattr(toggle, "aria-expanded") == "false"
    assert any(label.startswith("BOOK-") for label in labels)
    assert getattr(expanded, "data-risk-view-token") == "custom-test"
...
def test_pivot_filter_status_tracks_selection_without_a_self_trigger(tmp_path) -> None:
    options = [
        {"label": "Ford CDS", "value": "Ford CDS"},
        {"label": "GM CDS", "value": "GM CDS"},
    ]
    assert pivot_filter_status("reported underlying", options, ["Ford CDS"]) == (
        "1 selected; this limit applies immediately."
    )

    app = Dash("pivot-filter-callback-test", suppress_callback_exceptions=True)
    app.layout = dcc.Store(id="pivot-filter-test-layout")
    register_pivot_callbacks(app, object(), None, RiskViewRepository(tmp_path))
    client = app.server.test_client()
    dependencies = client.get("/_dash-dependencies").get_json()
    values_callback = next(
        callback
        for callback in dependencies
        if "risk-pivot-filter-values.options" in callback["output"]
    )
    value_dependency = ("risk-pivot-filter-values", "value")
    assert value_dependency not in {
        (item["id"], item["property"]) for item in values_callback["inputs"]
    }
    assert value_dependency in {
        (item["id"], item["property"]) for item in values_callback["state"]
    }
    status_inputs = {
        ...
        for item in next(
            callback
            for callback in dependencies
            if callback["output"] == "risk-pivot-filter-status.children"
        )["inputs"]
    }
    assert value_dependency in status_inputs
    assert ("risk-pivot-filter-values", "options") in status_inputs
    ...
    response = client.post(
        ...
        json={
            ...
            "changedPropIds": ["risk-pivot-filter-values.value"],
            "inputs": [
                {
                    "id": "risk-pivot-filter-field",
                    "property": "value",
                    "value": "reported underlying",
                },
                {
    ...
    assert response.get_json()["response"]["risk-pivot-filter-status"] == {
        "children": "1 selected; this limit applies immediately."
    }
...
def test_risk_explorer_has_exact_v4_tabs_and_page_owned_custom_controls() -> None:
    ...
    assert {
        ...
        "risk-pivot-measures",
        "risk-pivot-apply",
        "risk-pivot-filter-status",
        "risk-pivot-dirty-status",
        "risk-custom-open-rows",
        "risk-custom-view-token",
        "risk-custom-grid",
    } <= component_ids
