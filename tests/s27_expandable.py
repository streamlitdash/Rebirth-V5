...
def test_table_borders_are_solid_and_hierarchy_uses_width_for_emphasis() -> None:
    ...
    assert "border-top: 2px solid var(--semantic-rule)" in stylesheet
    assert "border-top: 1px solid var(--semantic-rule)" in stylesheet
...
def test_quick_risk_client_sync_preserves_canonical_disclosure_state() -> None:
    source = (_ROOT / "assets" / "s13_risk.js").read_text(encoding="utf-8")
