...
def test_sql_pl_repository_matches_projected_history_and_stays_bounded(
    ...
    pandas_summary = summarize_visible_pl_history(
        expected,
        open_path_tokens=[pl_history_path_token((first_signoff,))],
    )

    def ordered_summary(frame: pd.DataFrame) -> pd.DataFrame:
        return (
            frame.assign(_path_order=frame["Hierarchy Path"].map(repr))
            .sort_values("_path_order", kind="stable")
            .drop(columns="_path_order")
            .loc[:, list(PL_HISTORY_SUMMARY_COLUMNS)]
            .reset_index(drop=True)
        )

    pd.testing.assert_frame_equal(
        ordered_summary(sql_summary),
