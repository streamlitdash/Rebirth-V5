...
from cube.domain.s09_stock import STOCK_COLUMNS
...
def _resolved_date_range(
    ...
    *,
    ...
    if selected == "wtd":
        ...
        start, end = maximum_value - pd.DateOffset(years=1), maximum_value
...
class SQLPLHistoryRepository:
    ...
    def _new_temporary_directory() -> tempfile.TemporaryDirectory[str]:
        temporary = tempfile.TemporaryDirectory(prefix="cube-pnl-duckdb-")
    ...
    def series(
        ...
        *,
        ...
        if selected_preset not in _SERIES_PRESETS:
            selected_preset = "all"
        page_clause, page_parameters = _filter_clause(
            filters,
            exclude_selected=exclude_selected,
        )
        criteria_clause, criteria_parameters = _filter_clause(
            criteria,
            exclude_selected=False,
        )
        clause = f"({page_clause}) AND ({criteria_clause})"
        parameters = [*page_parameters, *criteria_parameters]
        try:
            with (
                self._lock,
                perf_span(
                ...
                minimum = days[0].snapshot_date
                maximum = days[-1].snapshot_date
