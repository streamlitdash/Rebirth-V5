...
def _identity_candidates(value: str) -> tuple[str, ...]:
    """Match current temp identities and immutable v4 archive identities."""

    selected = str(value)
    candidates = [selected]
    if _TEMP_NOTICE in selected:
        candidates.append(selected.replace(_TEMP_NOTICE, _LEGACY_ARCHIVE_NOTICE))
    elif _LEGACY_ARCHIVE_NOTICE in selected:
        candidates.append(selected.replace(_LEGACY_ARCHIVE_NOTICE, _TEMP_NOTICE))
    return tuple(dict.fromkeys(candidates))


def _normalize_legacy_identities(frame: pd.DataFrame) -> pd.DataFrame:
    """Present legacy fixture labels through the current temp-data contract."""
    ...
    for column in result.columns:
        ...
        result[column] = values.map(
            lambda value: (
                value.replace(_LEGACY_ARCHIVE_NOTICE, _TEMP_NOTICE)
...
class ArchiveSQLStore:
    ...
    def catalog_frames(self, generation: str) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        with self._lock:
            ...
            risk = _normalize_legacy_identities(risk)
            market = _normalize_legacy_identities(market)
            self._catalog_generation = generation
    ...
    def available_dates(
        ...
        *,
        ...
        identities = _identity_candidates(underlying)
