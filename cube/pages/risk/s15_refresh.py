"""V5 Risk-page startup and refresh callback ownership."""
...
_AUTOMATIC_REFRESH_COALESCE_WINDOW = timedelta(minutes=14)


def _automatic_refresh_due(
    snapshot: ControlSnapshotProtocol | RefreshSnapshotProtocol,
    *,
    now: datetime | None = None,
) -> bool:
    """Coalesce browser-local auto ticks after another browser's auto attempt."""

    if str(getattr(snapshot, "refresh_reason", "") or "") != (
        _AUTOMATIC_REFRESH_REASON
    ):
        return True
    last_attempt = getattr(snapshot, "last_attempt_at", None)
    ...
    return (
        current.astimezone(timezone.utc) - last_attempt.astimezone(timezone.utc)
        >= _AUTOMATIC_REFRESH_COALESCE_WINDOW
    )
...
def register_refresh_callbacks(
    ...
    *,
    ...
    def synchronize_committed_dashboard(
        snapshot: ControlSnapshotProtocol | RefreshSnapshotProtocol,
    ) -> tuple[
        ControlSnapshotProtocol | RefreshSnapshotProtocol,
        Any,
    ]:
        """Publish only the dashboard frame for the same committed revision."""

        if int(cache.revision) == int(snapshot.revision):
            return snapshot, cache.current(None)
        snapshot_frame = getattr(snapshot, "dashboard_frame", None)
        if snapshot_frame is not None:
            return snapshot, cache.replace_frame(snapshot_frame, snapshot.revision)
        for _attempt in range(2):
            dashboard_read = refresh_manager.read_frame("dashboard_frame")
            if int(dashboard_read.revision) == int(snapshot.revision):
                return snapshot, cache.replace_frame(
    ...
    def materialize_initial_dashboard(
        ...
        try:
            snapshot, prepared = synchronize_committed_dashboard(
                snapshot,
            )
            layout = build_layout(
                prepared,
    ...
    if refresh_manager is not None:
        app.clientside_callback(
            ...
            State("refresh-view-risk-explorer", "data", allow_optional=True),
            State("refresh-view-unmapped-books", "data", allow_optional=True),
            State("refresh-view-aggregate-pl", "data", allow_optional=True),
            State("refresh-view-top-promotions", "data", allow_optional=True),
            State("refresh-view-quick-risk-options", "data", allow_optional=True),
            State("refresh-view-quick-risk-table", "data", allow_optional=True),
            State("refresh-view-quick-risk-chart", "data", allow_optional=True),
            State("refresh-view-quick-market-options", "data", allow_optional=True),
            State("refresh-view-quick-market", "data", allow_optional=True),
