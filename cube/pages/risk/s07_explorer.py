"""V5 Risk Explorer callback ownership."""
...
def _explorer_option_state(values: Sequence[str] | None) -> tuple[bool, bool, bool]:
    """Return promotion, region, and reduced-tenor flags from one control."""

    selected = {str(value) for value in (values or ())}
    return (
        "promotion" in selected,
        "region" in selected,
        "reduced-tenor" in selected,
    )
...
def register_explorer_callbacks(
    ...
    def reduce_and_render_risk_view(
        ...
        promotion_enabled, region_enabled, reduced_tenor = _explorer_option_state(
            explorer_options
        )
        normalized_ir_family = ir_family if active_risk_type == "IR" else None
        context = {
