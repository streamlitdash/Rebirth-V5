...
from cube.domain.s04_crossgamma import XGAMMA_SOURCE_RISK_GREEKS
...
class ProductConnectorAdapter:
    ...
    ``market_open(open_date, underlying, *, market_status)``
        ...
        Risk. An empty DataFrame with the correct columns is allowed and means
        that the opening leg is unavailable. ``market_status`` is supplied by
        the manager and is exactly ``Live`` or ``OFFICIAL``; connectors must not
        infer it again.
    ...
    ``market_open_bulk`` / ``market_status_bulk``
