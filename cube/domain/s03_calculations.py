...
from cube.domain.s01_schema import TENOR_OPTION, TENOR_ORDER_COLUMNS, TENOR_SWAP
...
_EXPLICIT_NONFINITE_TOKENS = frozenset(
    {
        "na",
        "n/a",
        "nan",
...
def _true_blank_mask(values: pd.Series) -> pd.Series:
    """Identify absent source cells without treating explicit NA text as blank."""

    return (values.isna() | values.astype("string").str.strip().eq("")).fillna(True)


def _coerce_risk_measures(
    frame: pd.DataFrame,
    columns: list[str],
    label: str,
) -> pd.DataFrame:
    """Normalize Risk/dRisk measures while retaining genuinely blank cells.

    Explicit NA/infinity tokens and numeric infinities are sourced values, so
    they become zero. Empty cells remain missing and can be ignored according
    to the owning measure's contract. Other non-numeric values still indicate
    a malformed connector response.
    """
    ...
    for column in columns:
        ...
        blank = _true_blank_mask(values)
        text = values.astype("string").str.strip().str.casefold()
        explicit_text = text.isin(_EXPLICIT_NONFINITE_TOKENS).fillna(False) & ~blank
        numeric = pd.to_numeric(values.mask(blank), errors="coerce")
        numeric_nonfinite = numeric.notna() & ~np.isfinite(numeric)
        invalid = boolean | (~blank & numeric.isna() & ~explicit_text)
        if invalid.any():
            rows = result.index[invalid].tolist()[:5]
            raise ValueError(
                f"{label} column {column!r} has non-numeric values at rows {rows}"
            )
        normalized = numeric.astype(float)
        normalized.loc[explicit_text | numeric_nonfinite] = 0.0
        normalized.loc[blank] = np.nan
        result[column] = normalized
    return result


def _coerce_nullable_quote(
    frame: pd.DataFrame,
    column: str,
    label: str,
) -> pd.DataFrame:
    """Coerce a quote while allowing a genuinely absent market leg."""
    ...
    blank = _true_blank_mask(values)
    numeric = pd.to_numeric(values.mask(blank), errors="coerce")
    invalid = boolean | (~blank & (numeric.isna() | ~np.isfinite(numeric)))
    if invalid.any():
        rows = result.index[invalid].tolist()[:5]
        raise ValueError(
            f"{label} column {column!r} must be blank or contain finite numeric "
...
def _validate_market_tenor_orders(
    ...
    """Validate rank values; mapping disagreements are reconciled after the join."""
    result = frame.copy()
...
def _modal_smallest_rank(values: pd.Series) -> int | None:
    """Choose one deterministic supplied rank, preferring the mode then minimum."""

    ranks = pd.to_numeric(values, errors="coerce").dropna().astype(int)
    if ranks.empty:
        return None
    counts = ranks.value_counts(sort=False)
    largest = counts.max()
    return int(min(counts.index[counts.eq(largest)]))


def _reconcile_market_tenor_orders(
    market: pd.DataFrame,
    spec: ProductSpec,
) -> pd.DataFrame:
    """Resolve Open/Current order disagreements into one stable axis mapping.

    Open is the preferred authority for a tenor label. Current supplies a rank
    only when Open does not. A rank collision between labels causes that one
    Underlying/axis vocabulary to be densely renumbered in preferred-rank and
    stable source order; non-colliding connector ranks remain unchanged.
    """
    ...
    for axis in spec.axes:
        ...
        resolved = pd.Series(pd.NA, index=result.index, dtype="Int64")

        for _underlying, positions in result.groupby(
            UNDERLYING,
            sort=False,
            dropna=False,
        ).groups.items():
            group_index = pd.Index(positions)
            labels = result.loc[group_index, tenor_column].astype(str)
            unique_labels = labels.drop_duplicates().tolist()
            authority: list[tuple[str, int, int, int]] = []
            for first_seen, tenor_label in enumerate(unique_labels):
                label_index = labels.index[labels.eq(tenor_label)]
                preferred_open = _modal_smallest_rank(
                    result.loc[label_index, open_order]
                )
                preferred_status = _modal_smallest_rank(
                    result.loc[label_index, status_order]
                )
                preferred_rank = (
                    preferred_open if preferred_open is not None else preferred_status
                )
                if preferred_rank is None:  # pragma: no cover - validated legs
                    preferred_rank = first_seen
                authority.append(
                    (
                        tenor_label,
                        preferred_rank,
                        0 if preferred_open is not None else 1,
                        first_seen,
                    )
                )

            supplied_ranks = [rank for _label, rank, _source, _seen in authority]
            if len(supplied_ranks) != len(set(supplied_ranks)):
                ordered = sorted(
                    authority,
                    key=lambda item: (
                        item[1],
                ...
                rank_by_label = {
                    tenor_label: rank
                    for rank, (tenor_label, _old, _source, _seen) in enumerate(ordered)
                }
            else:
                rank_by_label = {
                    tenor_label: rank for tenor_label, rank, _source, _seen in authority
                }
            resolved.loc[group_index] = labels.map(rank_by_label).astype("Int64")

        result[order_column] = resolved
        result = result.drop(columns=[open_order, status_order])
    return result
...
def get_risk(
    ...
    # APP DATA ACCESS IS CENTRALIZED IN cube.services.s05_sources.get_risk.
...
def get_market_open(
    ...
    *,
    ...
    # APP DATA ACCESS IS CENTRALIZED IN cube.services.s05_sources.get_market_open.
    # Direct-library integration shape (comments only, never fallback data):
...
def get_market_status(
    ...
    *,
    ...
    # APP DATA ACCESS IS CENTRALIZED IN cube.services.s05_sources.get_market_status.
    # Direct-library integration shape (comments only, never fallback data):
...
def get_product_risk(
    ...
    frame = _coerce_risk_measures(frame, [RISK, DRISK], f"{spec.key} risk")
    # A blank authoritative Risk is not a zero exposure. Ignore that position;
    # a blank dRisk remains on an otherwise valid position as unavailable data.
    frame = frame.loc[frame[RISK].notna()].copy()
    frame = _coerce_numeric(frame, [VOL_SCORE], f"{spec.key} risk")
...
def get_product_market_open(
    ...
    frame = _coerce_nullable_quote(frame, OPEN, f"{spec.key} market open")
...
def get_product_market_status(
    ...
    *,
    ...
    frame = _coerce_nullable_quote(frame, CURRENT, f"{spec.key} current market")
...
def _merge_validated_market_legs(
    ...
    *,
    ...
    # Order fields describe display authority, not quote identity. Resolve all
    # disagreements only after joining the quote legs on canonical market keys.
    market = _reconcile_market_tenor_orders(market, spec)
    ...
    copied_open = market[OPEN].isna() & market[CURRENT].notna()
    copied_current = market[CURRENT].isna() & market[OPEN].notna()
    market.loc[copied_open, OPEN] = market.loc[copied_open, CURRENT]
    market.loc[copied_current, CURRENT] = market.loc[copied_current, OPEN]
    market[MARKET_AVAILABLE] = market[OPEN].notna() & market[CURRENT].notna()
...
def _pnl_move(spec: ProductSpec, frame: pd.DataFrame) -> pd.Series:
    ...
    if spec.pl_formula == "percentage":
        ...
        percentage_move = raw_move / nonzero_open
        # A zero quote copied across a missing leg is still an observed zero
        # move. Preserve that continuity instead of manufacturing 0 / 0 NaN.
        return percentage_move.mask(frame[OPEN].eq(0.0) & raw_move.eq(0.0), 0.0)
    return raw_move


def _normalize_computed_pl(frame: pd.DataFrame) -> pd.DataFrame:
    """Zero nonfinite calculations while preserving truly unavailable P&L."""

    result = frame.copy()
    numeric = pd.to_numeric(result[PL], errors="coerce").astype(float)
    available = result[MARKET_AVAILABLE].fillna(False).astype(bool)
    calculated_nonfinite = available & (numeric.isna() | ~np.isfinite(numeric))
    numeric.loc[calculated_nonfinite] = 0.0
    numeric.loc[~available] = np.nan
    result[PL] = numeric
    return result


def _normalize_developed_risk(
    values: pd.Series,
    market_available: pd.Series,
) -> pd.Series:
    """Apply the nonfinite Risk policy to a calculated Taylor exposure."""

    numeric = pd.to_numeric(values, errors="coerce").astype(float)
    available = market_available.fillna(False).astype(bool)
    numeric.loc[available & (numeric.isna() | ~np.isfinite(numeric))] = 0.0
    numeric.loc[~available] = np.nan
    return numeric
...
def get_product_pl(
    ...
    *,
    ...
    zero_open_with_move = (
        result[MARKET_AVAILABLE]
        & result[OPEN].eq(0.0)
        & result[CURRENT].ne(result[OPEN])
    )
    invalid_percentage_open = (
        spec.pl_formula == "percentage" and zero_open_with_move.any()
    )
    if invalid_percentage_open:
        result.loc[zero_open_with_move, MARKET_DATA_STATUS] = (
    ...
    if spec.pl_formula != "taylor_gamma":
        ...
        result = _normalize_computed_pl(result)
        result[SPLIT] = "Risk"
        return result
    ...
    developed_risk = _normalize_developed_risk(
        developed_risk,
        result[MARKET_AVAILABLE],
    )
    with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
        sourced[PL] = 0.5 * developed_risk * taylor_move * validated_multiplier
    sourced = _normalize_computed_pl(sourced)
