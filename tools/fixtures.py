...
from rebirth.domain.schema import (  # noqa: E402 - support execution from any directory
...
from rebirth.domain.products import (  # noqa: E402 - support execution from any directory
...
from rebirth.domain.stock import (  # noqa: E402 - support execution from any directory
    STOCK_COLUMNS,
...
def _history_quote_catalog() -> pd.DataFrame:
    ...
    for source_index, source_type in enumerate(HISTORY_SOURCE_TYPES):
        ...
        for local_index in range(source_rows):
            ...
            underlying = _fake(f"{base_underlying} · Series {family_index + 1:03d}")
            if len(spec.axes) == 2:
            ...
            rows.append(
                {
                    ...
                    "Reported Underlying": _fake(
                        f"{spec.risk_type} {displayed_greek} Family "
                        f"{underlying_index + 1:03d}"
                    ),
...
def _build_history_risk(
    ...
    *,
    ...
    frame["Split"] = "Risk"
    derived_gamma = repeated[SOURCE_TYPE].isin(_DERIVED_GAMMA_SOURCE)
...
def validate_official_history_fixture(
    ...
    *,
    ...
    for gamma_source, delta_source in _DERIVED_GAMMA_SOURCE.items():
        ...
        _require(
            gamma_risk["Risk Greek"].eq("Delta").all()
            and gamma_risk["Split"].eq("Gamma").all()
            and delta_risk["Risk Greek"].eq("Delta").all()
            and delta_risk["Split"].eq("Risk").all(),
            f"Derived {gamma_source} Risk rows must contribute as Delta/Gamma",
        )
        _require(
        ...
        shared_reported = set(gamma_risk["Reported Underlying"]) & set(
        ...
        shared_raw_axes = set(
            gamma_risk[["Underlying", *active_axes]].itertuples(
                index=False,
                name=None,
            )
        ) & set(
    ...
    quote_keys = [SOURCE_TYPE, "Risk Type", "Risk Greek", "Underlying", *TENOR_COLUMNS]
